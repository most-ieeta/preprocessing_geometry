...
turning_function get_turning_function(const std::vector<SimplePoint> &poly) {
  turning_function ret;
  double p = 0.0;

  SimplePoint last = poly[poly.size() - 1]; // Close the loop

  for (const SimplePoint pt : poly) {
 ...
 * */
std::pair<size_t, size_t> best_fit(const std::vector<SimplePoint> &f1,
                                   const std::vector<SimplePoint> &f2,
                                   double *out_area = nullptr) {

  std::pair<size_t, size_t> ret;
  double min_area = 50000;

  std::vector<SimplePoint> src = f1;
  std::vector<SimplePoint> dst = f2;
 ...
 * red_poly
 */
std::vector<SimplePoint> reduce_best_fit(const std::vector<SimplePoint> &ref_poly,
                                   const std::vector<SimplePoint> &red_poly) {
  std::vector<SimplePoint> result;
  double min_area = 100000; //"Infinite" constant TODO

  for (size_t i = 0; i < red_poly.size(); ++i) {
    double area;
    std::vector<SimplePoint> current_iteration = red_poly;
...
int main(int argc, char *argv[]) {
  ...
  std::vector<SimplePoint> polys[2];
  {
    SimplePoint i;
