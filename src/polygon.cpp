...
double Polygon::distance_to_point(const SimplePoint& pt) const {
	double min = SimplePoint::norm(pt, SimplePoint::get_closest_point(pt, points[0], points[points.size()-1]));
	for (size_t i = 1; i < points.size(); ++i) {
		double dist = SimplePoint::norm(pt, SimplePoint::get_closest_point(pt, points[i], points[i-1]));
